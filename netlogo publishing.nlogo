...
agents-own
[
  exposed?        ;; has been exposed to idea
  belief          ;; private belief
  active-pro?     ;; willing to act in favour of belief
  active-con?     ;; willing to act against belief
  influence       ;; ability to persuade others to share belief
  suggestibility  ;; susceptibility to being persuaded by others
  papers-authored ;; list of publications published (patch position)
  papers-read     ;; list of papers exposed to (patch position)
...
[
  ...
  evidence        ;; strength of evidence in favour of belief
  subculture      ;; acceptance of belief in patch and neighbours
]

globals
[
  instigator      ;; first person to publish in favour of belief
  culture         ;; acceptance of belief among whole population
]
...
to read-publications
  ask agents [
    let visible (patch-set patch-here neighbors)
    ifelse exposed? 
      [if any? visible with [publication?]   ;; each agent checks for publication in neighbouring patches
        [let new-pubs visible with [publication?]
          let read []
          ask new-pubs [ set read sentence read self ]
          foreach read [
            if not member? ? papers-read [
              set papers-read sentence papers-read ?
            ]
          ]
...
to move
  ask agents [
    right random 360
    forward 1                               ;; each agent moves randomly
  ]
end
